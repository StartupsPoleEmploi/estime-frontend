...
export class ModificationCriteresComponent {
  ...
  @ViewChild('maSituationComponent', { read: MaSituationComponent }) maSituationComponent: MaSituationComponent;
  @ViewChild('activiteRepriseComponent', { read: ActiviteRepriseComponent }) activiteRepriseComponent: ActiviteRepriseComponent;
  ...
  public onSubmitModificationCriteresForm(): void {
    this.maSituationComponent.onSubmitMaSituationForm();
    this.activiteRepriseComponent.onSubmitActiviteRepriseForm();
  ...
  private traiterRetourSimulerMesAides(simulation: Simulation): void {
    ...
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => this.router.navigate([RoutesEnum.PARCOURS_COMPLEMENT_ARE, RoutesEnum.RESULTAT_SIMULATION]));
  }
