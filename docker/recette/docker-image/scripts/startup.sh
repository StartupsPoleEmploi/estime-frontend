...
function replace_properties_in_main {
  ...
  perl -i -p -e "s|pe-connect.client-id|${PE_CONNECT_CLIENT_ID_R7}|g" /var/www/estime/main*.js
  perl -i -p -e "s|pe-connect.redirect-uri|${PE_CONNECT_REDIRECT_URI_R7}|g" /var/www/estime/main*.js
  perl -i -p -e "s|pe-connect.scope|${PE_CONNECT_SCOPE_R7}|g" /var/www/estime/main*.js
  perl -i -p -e "s|pe-connect.identity-server-url|${PE_CONNECT_IDENTITY_SERVER_URL_R7}|g" /var/www/estime/main*.js
}
